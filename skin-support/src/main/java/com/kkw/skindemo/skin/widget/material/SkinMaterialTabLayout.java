...
public class SkinMaterialTabLayout extends TabLayout implements SkinCompatSupportable {
    ...
    public SkinMaterialTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray ta = context.obtainStyledAttributes(tabTextAppearance, com.kkw.skindemo.skin.R.styleable.SkinTextAppearance);
        try {
            mTabTextColorsResId = ta.getResourceId(com.kkw.skindemo.skin.R.styleable.SkinTextAppearance_android_textColor, INVALID_ID);
        } finally {
            ta.recycle();
        }
